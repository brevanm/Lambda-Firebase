...
WORKDIR /src
COPY ["Lambda-Firebase.csproj", "Lambda-Firebase/"]
RUN dotnet restore "Lambda-Firebase/Lambda-Firebase.csproj"

WORKDIR "/src/Lambda-Firebase"
COPY . .
RUN dotnet build "Lambda-Firebase.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Lambda-Firebase.csproj" \
...
Execute unit tests
```
cd "Lambda-Firebase/test/Lambda-Firebase.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "Lambda-Firebase/src/Lambda-Firebase"
